...
class UnstructuredHTMLParser(Parser):
    """Parses HTML text into Page objects."""
    ...
    async def parse(self, content: IO) -> AsyncGenerator[Page, None]:
        ...
        """
        if self.verbose:
            print(f"\tExtracting text from '{content.name}' using local HTML parser (Unstructured)")

        data = content.read()

        unstructured_kwargs ={"include_page_breaks":False}

        elements = partition_html(file = data,**unstructured_kwargs)

        result =  "\n\n".join([str(e) for e in elements])



        yield Page(0, 0, text=group_broken_paragraphs(result))
